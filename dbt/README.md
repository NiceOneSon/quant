...
## 레이어 (raw → staging → intermediate → marts)
- `source('raw')` — 원본 parquet (`_sources.yml`, 위치는 env_var).
- `models/staging/` — 원본 1:1 정리. **view only**. `stg_prices`·`stg_rates`·`stg_universe`.
- `models/intermediate/` — 조인·PIT 로직. **ephemeral**(마트에 인라인, 미노출).
  `int_prices_pit`(가격×멤버십 as-of) · `int_rates_enriched`(+seed 라벨) · `int_universe_membership`.
